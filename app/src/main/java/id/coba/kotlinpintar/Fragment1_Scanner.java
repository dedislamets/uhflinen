...
public class Fragment1_Scanner extends Fragment implements OnCheckedChangeListener,OnClickListener{
            ...
            switch (msg.what){
                case 1:
                    ...
                    if (epcSet == null) {//first add
                        ...
                        Util.play(1, 0);
                        IntentBiasaActivity.mSetEpcs=epcSet;
                    }else{
...
//		Log.e("f1","destroy view");
        if (isStart) {
            isStart = false;
            isRunning = false;
            IntentBiasaActivity.mUhfrManager.stopTagInventory();
        }
...
//		Log.e("hidden", "hide"+hidden) ;
        ...
        if (IntentBiasaActivity.mUhfrManager!=null) IntentBiasaActivity.mUhfrManager.setCancleInventoryFilter();
    }
            ...
            while(isRunning){
                if (isStart) {
                    ...
                    if (isMulti) { // multi mode
                        list1 = IntentBiasaActivity.mUhfrManager.tagInventoryRealTime();
                    }else{
...
//							e.printStackTrace();
//						}
                        list1 = IntentBiasaActivity.mUhfrManager.tagInventoryByTimer((short)50);
                        //inventory epc + tid
//                        list1 = IntentBiasaActivity.mUhfrManager.tagEpcTidInventoryByTimer((short) 50);
                    }
...
//                    }
    ...
    private void runInventory() {
        if (keyControl) {
            keyControl = false;
            if (!isStart) {
                IntentBiasaActivity.mUhfrManager.setCancleInventoryFilter();
                isRunning = true;
                if (isMulti) {
                    IntentBiasaActivity.mUhfrManager.setFastMode();
                    IntentBiasaActivity.mUhfrManager.asyncStartReading();
                }else {
                    IntentBiasaActivity.mUhfrManager.setCancleFastMode();
                }
...
//            Log.e("inventoryTask", "start inventory") ;
                ...
                if (isMulti)
                    IntentBiasaActivity.mUhfrManager.asyncStopReading();
                else
                    IntentBiasaActivity.mUhfrManager.stopTagInventory();
                try {
                    Thread.sleep(100);
    ...
    private void clearEpc(){
        ...
        allCount = 0 ;
        tvTagSum.setText("0");
        IntentBiasaActivity.mSetEpcs.clear();
