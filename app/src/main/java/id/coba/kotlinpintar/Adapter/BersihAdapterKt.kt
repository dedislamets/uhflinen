...
class BersihAdapterKt @Inject constructor(): RecyclerView.Adapter<BersihAdapterKt.UsersViewHolder>(){
    private var list = ArrayList<DataKotor>()
    var onItemClick: ((DataKotor) -> Unit)? = null

    inner class UsersViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        fun bind(data: DataKotor){
            with(itemView){
    ...
    fun addList(items: ArrayList<DataKotor>){
        list.addAll(items)
